...
//Point(p++) = {0.2273,0.0270645,0.0,basesize};
...
//Top
l_nozzle_top = l++;
// BSpline(l_nozzle_top) = {p_nozzle_top_start:p_nozzle_top_end};  // goes clockwise
Spline(l_nozzle_top) = {p_nozzle_top_start:p_nozzle_top_end};  // goes clockwise

//Bottom Lines
l_nozzle_bottom = l++;
// BSpline(l_nozzle_bottom) = {p_nozzle_bottom_start:p_nozzle_bottom_end}; // goes counter-clockwise
...
//Inlet
l_inlet = l++;
//l_inlet_upper_half = l++;
//l_inlet_lower_half = l++;
Line(l_inlet) = {p_nozzle_top_start,p_nozzle_bottom_start};  //goes counter-clockwise
//Line(l_inlet_upper_half) = {p_nozzle_top_start,p_inlet_midpoint};
//Line(l_inlet_lower_half) = {p_inlet_midpoint,p_nozzle_bottom_start};
...
p_outlet_lower = p++;
p_outlet_upper = p++;
//Point(???) = {0.872163,-0.0083245,0.0,basesize};
//Point(???) = {0.872163,0.01167548819733,0.0,basesize};
...
Field[14].DistMin = 0.00005;
Field[14].DistMax = 0.01;
